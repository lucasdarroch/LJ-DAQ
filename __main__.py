...
ts = [0,0,0,0];ts[0] = datetime.utcnow().timestamp();ts[3] = datetime.utcnow().timestamp()
...
# Launch buffer
filename = os.path.join(DATA,'%s%i.csv'%(fstr,ts[0]))
subprocess.run('python3 __buffer__.py %s %s %s %s'%(DATA,fstr,chunkTime,scanRate),shell=True)
...
def buffer_output():
    return DATA, fstr, chunkTime, scanRate, sensor['names']
...
if __name__ == '__main__':
    while True:
        ...
        ts[1] = datetime.utcnow().timestamp()
        ...
        ts[2] = datetime.utcnow().timestamp()
        procTime = ts[2]-ts[1]
